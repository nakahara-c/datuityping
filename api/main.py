...
from values_set import values_set, probabilities_set, id_set
...
def datui_randomImgID(request):
    ...
    if input_value < 1 or input_value > 6:
    ...
    imgID = id_set[level][random_value % 20 - 1]
    
    response = make_response(jsonify({"randomValue": random_value, "imgID": imgID}), 200)
